...
<body>
  <main class="shell">
    <section class="topbar">
      ...
      <div class="top-actions">
        ...
        <select class="conversation-select" id="conversationSelect" title="Conversaciones guardadas"></select>
        <button class="secondary" type="button" id="newConversationBtn"><i data-lucide="plus"></i>Nueva</button>
    ...
    <section class="grid">
      <section class="panel chat">
        ...
        <button class="scroll-bottom" type="button" id="scrollBottomBtn" title="Volver al ultimo mensaje"><i data-lucide="arrow-down"></i></button>
  ...
  <script>
    ...
    const conversationSelectEl = document.querySelector("#conversationSelect");
    ...
    let activeConversationId = localStorage.getItem("kb_conversation_id") || "";
    ...
    function renderConversationMessages(messages) {
      ...
      for (const item of messages || []) {
        ...
        addAssistantMessage(item.answer || "", item.images || [], item.id || null);
      }
      scrollChatToBottom();
    }

    async function loadConversations(selectedId = activeConversationId, renderMessages = false) {
      if (!userToken) return;
      ...
      const data = await response.json();
      conversationSelectEl.innerHTML = "";
      const firstOption = document.createElement("option");
      firstOption.value = "";
      firstOption.textContent = "Nueva conversacion";
      conversationSelectEl.appendChild(firstOption);
      for (const item of data.conversations || []) {
        const option = document.createElement("option");
        option.value = item.id;
        option.textContent = `${item.title} (${item.messages || 0})`;
        conversationSelectEl.appendChild(option);
      }
      activeConversationId = selectedId || "";
      conversationSelectEl.value = activeConversationId;
      if (renderMessages) renderConversationMessages(data.messages || []);
    }
    ...
    function renderAdminOverview(data) {
      ...
      renderList(adminUsersListEl, data.admin_users || [], "No hay usuarios configurados.", (row, item) => {
        ...
        row.querySelector("span").textContent = `${item.role_label || item.role} | ${item.status} | ${item.source === "txt" ? "usuarios.txt" : "base local"}`;
    ...
    function setChatLocked(locked) {
      ...
      conversationSelectEl.disabled = locked;
      newConversationBtn.disabled = locked;
    ...
    async function logoutUser() {
      ...
      localStorage.removeItem("kb_conversation_id");
      conversationSelectEl.innerHTML = "";
      clearChat();
    ...
    if (!activeUserName || !userToken) {
      ...
      loadConversations(activeConversationId, Boolean(activeConversationId));
    }
    ...
    async function saveUserName() {
      ...
      setChatLocked(false);
      await loadConversations(activeConversationId, Boolean(activeConversationId));
      if (!activeConversationId) addAssistantMessage(randomGreeting(activeUserName));
      questionEl.focus();
    }
    ...
    messagesEl.addEventListener("scroll", () => {
      ...
      scrollBottomBtn.classList.toggle("visible", distance > 160);
    });
    conversationSelectEl.addEventListener("change", async () => {
      activeConversationId = conversationSelectEl.value;
      if (activeConversationId) {
        localStorage.setItem("kb_conversation_id", activeConversationId);
        await loadConversations(activeConversationId, true);
      } else {
        localStorage.removeItem("kb_conversation_id");
        clearChat();
        addAssistantMessage(randomGreeting(activeUserName || "usuario"));
      }
    });
    newConversationBtn.addEventListener("click", () => {
      activeConversationId = "";
      localStorage.removeItem("kb_conversation_id");
      conversationSelectEl.value = "";
      clearChat();
      addAssistantMessage(randomGreeting(activeUserName || "usuario"));
      questionEl.focus();
    });
    ...
    askForm.addEventListener("submit", async (event) => {
      ...
      const response = await fetch("/api/ask", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ question, user_name: userName, token: userToken, conversation_id: activeConversationId, ...browserLocalMeta() })
      });
      const data = await response.json();
      waiting.remove();
      ...
      if (data.conversation_id) {
        ...
        localStorage.setItem("kb_conversation_id", activeConversationId);
        await loadConversations(activeConversationId, false);
      }
...
def normalize_role(role: str) -> str:
    ...
    if value in {"1", "super", "superusuario", "super_usuario", "admin", "administrador"}:
        return "super_usuario"
    if value in {"2", "invitado", "usuario", "user"}:
        return "invitado"
...
def users_file_path() -> Path:
    return USERS_FILE_PATH.expanduser()


def parse_users_file() -> list[dict]:
            ...
            users.append(
                {
                    "username": username,
                    "password": password,
                    "role": normalize_role(current.get("role", "2")),
                    "active": parse_active_flag(current.get("active", "1")),
                    "source": "txt",
                }
            )
        current = {}
    ...
    for line in raw.splitlines():
        ...
        field = aliases.get(normalize_field_name(key))
        if not field:
            continue
        if field == "username" and current.get("username"):
            flush()
...
def get_file_user(username: str) -> dict | None:
    wanted = str(username or "").strip().upper()
    if not wanted:
        return None
    for user in parse_users_file():
        if str(user.get("username", "")).strip().upper() == wanted:
...
def init_db() -> None:
    ...
    with sqlite3.connect(DB_PATH) as connection:
        ...
        if "conversation_id" not in columns:
            connection.execute("ALTER TABLE interactions ADD COLUMN conversation_id TEXT DEFAULT ''")
...
def list_app_users() -> list[dict]:
    ...
    merged = {str(row["username"]).upper(): {**dict(row), "source": "local"} for row in rows}
    for user in parse_users_file():
        now = local_now().isoformat(timespec="seconds")
        merged[str(user["username"]).upper()] = {
    ...
    return sorted(merged.values(), key=lambda item: str(item["username"]).lower())


def get_app_user(username: str) -> dict | None:
    file_user = get_file_user(username)
    if file_user:
        return file_user
    init_db()
    with sqlite3.connect(DB_PATH) as connection:
...
def app_user_password_ok(user: dict | None, password: str) -> bool:
    if not user:
        return False
    plain = user.get("password_plain")
    if plain is not None:
        return str(password) == str(plain)
...
def upsert_app_user(username: str, password: str = "", role: str = "invitado", active: bool = True, event_time: datetime | None = None) -> dict:
    ...
    if existing and existing.get("source") == "txt":
        raise UserFacingError("Este usuario viene de usuarios.txt. Editalo directamente en ese archivo.")
    with sqlite3.connect(DB_PATH) as connection:
        if existing is None:
            connection.execute(
                """
...
def ensure_conversation(user_name: str, conversation_id: str = "", first_question: str = "", event_time: datetime | None = None) -> str:
    init_db()
    now = (event_time or local_now()).isoformat(timespec="seconds")
    ...
    with sqlite3.connect(DB_PATH) as connection:
        ...
        if clean_id:
            row = connection.execute(
                "SELECT id FROM conversations WHERE id = ? AND UPPER(user_name) = UPPER(?)",
                (clean_id, user_name),
            ).fetchone()
            if row:
                connection.execute("UPDATE conversations SET updated_at = ? WHERE id = ?", (now, clean_id))
                return clean_id
        clean_id = secrets.token_urlsafe(12)
        connection.execute(
            """
            INSERT INTO conversations (id, user_name, title, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (clean_id, user_name, conversation_title(first_question), now, now),
        )
    return clean_id


def list_user_conversations(user_name: str, limit: int = 40) -> list[dict]:
    init_db()
    if not user_name:
        return []
    with sqlite3.connect(DB_PATH) as connection:
        ...
        rows = connection.execute(
            """
            SELECT c.id, c.title, c.created_at, c.updated_at, COUNT(i.id) AS messages
            FROM conversations c
            LEFT JOIN interactions i ON i.conversation_id = c.id
            WHERE UPPER(c.user_name) = UPPER(?)
...
def conversation_messages(user_name: str, conversation_id: str, limit: int = 60) -> list[dict]:
    init_db()
    ...
    with sqlite3.connect(DB_PATH) as connection:
        ...
        rows = connection.execute(
            """
            SELECT id, created_at, question, answer, images, rating, rating_note
            FROM interactions
            WHERE UPPER(user_name) = UPPER(?) AND conversation_id = ?
            ORDER BY id ASC
            LIMIT ?
            """,
            (user_name, conversation_id, limit),
        ).fetchall()
    result = []
    for row in rows:
        item = dict(row)
...
def log_interaction(user_name: str, question: str, answer: str, sources: list[dict], images: list[dict], diagnostics: dict | None = None, event_time: datetime | None = None, conversation_id: str = "") -> tuple[int, str]:
    init_db()
    now = event_time or local_now()
    conversation_id = ensure_conversation(user_name, conversation_id, question, now)
    ...
    with sqlite3.connect(DB_PATH) as connection:
        cursor = connection.execute(
            """
            INSERT INTO interactions (conversation_id, created_at, date, user_name, question, answer, sources, images, diagnostics)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                conversation_id,
    ...
    return interaction_id, conversation_id
...
def recent_user_history(user_name: str, limit: int = 20, conversation_id: str = "") -> list[dict]:
    init_db()
    if not user_name:
        return []
    with sqlite3.connect(DB_PATH) as connection:
        ...
        if conversation_id:
            rows = connection.execute(
                ...
                FROM interactions
                WHERE user_name = ? AND conversation_id = ?
                ORDER BY id DESC
                LIMIT ?
                """,
                (user_name, conversation_id, limit),
...
class AppHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if self.path == "/api/files":
            ...
            return
        if urlparse(self.path).path == "/api/conversations":
    ...
    def user_token_from_path(self) -> str:
        query = parse_qs(urlparse(self.path).query)
        return str(query.get("token", [""])[0]).strip()

    def handle_conversations(self) -> None:
        session = current_user_session(self.user_token_from_path())
        if not session.get("username"):
            json_response(self, {"ok": False, "message": "Sesion no valida."}, status=401)
            return
        conversation_id = str(parse_qs(urlparse(self.path).query).get("conversation_id", [""])[0]).strip()
        payload = {
            "ok": True,
            "conversations": list_user_conversations(str(session["username"])),
            "messages": conversation_messages(str(session["username"]), conversation_id) if conversation_id else [],
        }
    ...
    def handle_user_login(self) -> None:
        ...
        if user and user.get("source") != "txt" and not app_user_password_ok(user, password) and legacy_admin_password_ok(user["username"], password):
            upsert_app_user(user["username"], password=password, role=user["role"], active=bool(user["active"]), event_time=event_time)
            user = get_app_user(username)
        if not user or not user["active"] or not app_user_password_ok(user, password):
    ...
    def handle_ask(self) -> None:
        ...
        if not question:
            ...
            return
        try:
            history = recent_user_history(user_name, conversation_id=conversation_id)
        ...
        images = images[:4]
        interaction_id, conversation_id = log_interaction(user_name, question, result["answer"], result["sources"], images, diagnostics=diagnostics, event_time=event_time, conversation_id=conversation_id)
        json_response(self, {"ok": True, "answer": result["answer"], "images": images, "interaction_id": interaction_id, "conversation_id": conversation_id})
